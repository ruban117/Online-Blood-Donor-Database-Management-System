...
class Donordb{
    ...
    public function UpdateAccept($donemail,$reqemail)
    {
        $sql="UPDATE acceptance SET accepted = 0 WHERE donor_email = :donemail AND req_email = :reqemail";
        $stmt=$this->conn->prepare($sql);
        $stmt->execute(['donemail'=>$donemail,'reqemail'=>$reqemail]);
    }

    public function UpdateReject($donemail,$reqemail)
    {
        $sql="UPDATE acceptance SET accepted = 1 WHERE donor_email = :donemail AND req_email = :reqemail";
        $stmt=$this->conn->prepare($sql);
        $stmt->execute(['donemail'=>$donemail,'reqemail'=>$reqemail]);
    }
    ...
    public function Block($email){
        $sql2="UPDATE donor SET is_block = 1 WHERE email= :email";
        $stmt2=$this->conn->prepare($sql2);
